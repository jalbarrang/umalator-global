const pacemakerColors: Array<{ stroke: string; fill: string }> = [
  { stroke: '#22c55e', fill: 'rgba(34, 197, 94, 0.6)' }, // Green
  { stroke: '#a855f7', fill: 'rgba(168, 85, 247, 0.6)' }, // Purple
  { stroke: '#ec4899', fill: 'rgba(236, 72, 153, 0.6)' } // Pink
];

export const colors: Array<{ stroke: string; fill: string }> = [
  { stroke: 'rgb(42, 119, 197)', fill: 'rgba(42, 119, 197, 0.7)' },
  { stroke: 'rgb(197, 42, 42)', fill: 'rgba(197, 42, 42, 0.7)' }
];

export const rushedColors: Array<{ stroke: string; fill: string }> = [
  { stroke: 'rgb(42, 119, 197)', fill: 'rgba(42, 119, 197, 0.8)' }, // Blue for Uma 1
  { stroke: 'rgb(197, 42, 42)', fill: 'rgba(197, 42, 42, 0.8)' } // Red for Uma 2
];

export const recoveryColors: Array<{ stroke: string; fill: string }> = [
  { stroke: 'rgb(42, 119, 197)', fill: 'rgba(42, 119, 197, 0.82)' }, // Uma 1 (blue)
  { stroke: 'rgb(197, 42, 42)', fill: 'rgba(197, 42, 42, 0.82)' } // Uma 2 (red)
];

export const debuffColors: Array<{ stroke: string; fill: string }> = [
  { stroke: 'rgb(139, 92, 246)', fill: 'rgba(139, 92, 246, 0.76)' }, // Uma 1 (purple)
  { stroke: 'rgb(236, 72, 153)', fill: 'rgba(236, 72, 153, 0.76)' } // Uma 2 (pink)
];

export const fullyChargedColors: Array<{ stroke: string; fill: string }> = [
  { stroke: 'rgb(42, 119, 197)', fill: 'rgba(42, 119, 197, 0.7)' }, // Uma 1 (blue)
  { stroke: 'rgb(197, 42, 42)', fill: 'rgba(197, 42, 42, 0.7)' } // Uma 2 (red)
];

export const posKeepColors: Array<{ stroke: string; fill: string }> = [
  { stroke: 'rgb(42, 119, 197)', fill: 'rgba(42, 119, 197, 0.6)' },
  { stroke: 'rgb(197, 42, 42)', fill: 'rgba(197, 42, 42, 0.6)' }
];

export const rankColors: Array<{ stroke: string; fill: string }> = [
  { stroke: 'rgb(42, 119, 197)', fill: 'rgba(42, 119, 197, 0.25)' },
  { stroke: 'rgb(197, 42, 42)', fill: 'rgba(197, 42, 42, 0.25)' }
];
