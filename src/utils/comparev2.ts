...
export function runComparison(params: RunComparisonParams): CompareResult {
  ...
  const baseRunnerA = cloneDeep(runnerA);
  const baseRunnerB = cloneDeep(runnerB);
  ...
  // Force Sync the RNG for the umas so they both use the same RNG seed
  // if (options.mode === 'compare') {
  //   runnerARaceSolver.desync();
  // }

  // TODO: Make use of only one RaceSolver for handling multiple umas instead of having two separate RaceSolvers
  // This will require setting up a different approach for tracking the needed data for each uma.
  runnerARaceSolver.trackedRunner(baseRunnerA);
  runnerBRaceSolver.trackedRunner(baseRunnerB);
  ...
  const runnerABaseStats = buildBaseStats(baseRunnerA);
  ...
  const runnerBBaseStats = buildBaseStats(baseRunnerB);
  ...
  for (const id of runnerASortedSkills) {
    ...
    // If forced position, add skill at position and continue
    if (forcedPos) {
  ...
  for (let i = 0; i < nsamples; ++i) {
    ...
    while (!solverAFinished || !solverBFinished) {
      ...
      for (let j = 0; j < options.pacemakerCount; j++) {
        ...
        auxPacer.step(1 / 15);
        data.pacerVelocity[j].push(
          auxPacer.currentSpeed +
            (auxPacer.modifiers.currentSpeed.acc + auxPacer.modifiers.currentSpeed.err),
        );

        data.pacerPosition[j].push(auxPacer.pos);
        data.pacerTime[j].push(auxPacer.accumulatetime.t);
      }
      ...
      if (solverB.pos < course.distance) {
        ...
        data.time[runnerBIndex].push(solverB.accumulatetime.t);
        data.position[runnerBIndex].push(solverB.pos);

        data.velocity[runnerBIndex].push(currentVelocity);
        ...
        data.startDelay[runnerBIndex] = solverB.startDelay;
        data.rushed[runnerBIndex] = solverB.rushedActivations.slice();
        data.positionKeepModePositions[runnerBIndex] = solverB.positionKeepActivations.slice();

        // Update dueling stats
        if (solverB.competeFightStart != null) {
          data.duelingRegions[runnerBIndex] = [
        ...
        if (solverB.leadCompetitionStart != null) {
          data.spotStruggleRegions[runnerBIndex] = [
      ...
      if (solverA.pos < course.distance) {
        ...
        data.time[runnerAIndex].push(solverA.accumulatetime.t);
        data.position[runnerAIndex].push(solverA.pos);
        data.velocity[runnerAIndex].push(currentVelocity);
        ...
        data.startDelay[runnerAIndex] = solverA.startDelay;
        data.rushed[runnerAIndex] = solverA.rushedActivations.slice();
        data.positionKeepModePositions[runnerAIndex] = solverA.positionKeepActivations.slice();

        // Update dueling stats
        if (solverA.competeFightStart != null) {
          data.duelingRegions[runnerAIndex] = [
        ...
        if (solverA.leadCompetitionStart != null) {
          data.spotStruggleRegions[runnerAIndex] = [
    ...
    if (data.position[runnerBIndex].length <= data.position[runnerAIndex].length) {
      const runneAFrames = data.position[runnerBIndex].length;
      positionDiff =
        data.position[runnerBIndex][runneAFrames - 1] -
        data.position[runnerAIndex][runneAFrames - 1];
    } else {
      const runnerBFrames = data.position[runnerAIndex].length;
      positionDiff =
        data.position[runnerBIndex][runnerBFrames - 1] -
        data.position[runnerAIndex][runnerBFrames - 1];
    }
      ...
      if (p && p.pos < course.distance) {
        ...
        for (let pacemakerIndex = 0; pacemakerIndex < 3; pacemakerIndex++) {
          if (pacemakerIndex < pacers.length && pacers[pacemakerIndex] === p) {
            data.pacerVelocity[pacemakerIndex].push(
              p.currentSpeed + (p.modifiers.currentSpeed.acc + p.modifiers.currentSpeed.err),
            );
            data.pacerPosition[pacemakerIndex].push(p.pos);
            data.pacerTime[pacemakerIndex].push(p.accumulatetime.t);
          }
        }
      }
    });
    ...
    for (let j = 0; j < options.pacemakerCount; j++) {
      ...
      data.pacerPositionKeep[j] = p ? p.positionKeepActivations.slice() : [];
      if (p && p.leadCompetitionStart != null) {
        data.pacerSpotStruggle[j] = [
    ...
    data.skillActivations[0] = Object.fromEntries(runnerASkillActivations);
    data.skillActivations[1] = Object.fromEntries(runnerBSkillActivations);
