...
import type { CourseData, Phase } from './CourseData';
...
function distributionRandomFactory<Ts extends Array<unknown>>(
  ...
  return function (...args: [...clsArgs: Ts, o: Partial<Condition>]) {
    ...
    const policy =
      key in cache ? cache[key] : (cache[key] = Object.freeze(new cls(...(args as unknown as Ts))));
...
export const Conditions: { [cond: string]: Condition } = Object.freeze({
  ...
  base_power: valueFilter(
    (_: CourseData, horse: HorseParameters, extra: RaceParameters) => horse.power,
  ),
  base_speed: valueFilter(
    (_: CourseData, horse: HorseParameters, extra: RaceParameters) => horse.speed,
  ),
  base_stamina: valueFilter(
    (_: CourseData, horse: HorseParameters, extra: RaceParameters) => horse.stamina,
  ),
  base_guts: valueFilter(
    (_: CourseData, horse: HorseParameters, extra: RaceParameters) => horse.guts,
  ),
  base_wiz: valueFilter(
    (_: CourseData, horse: HorseParameters, extra: RaceParameters) => horse.wisdom,
  ),
  ...
  corner_count: valueFilter(
    (course: CourseData, _: HorseParameters, extra: RaceParameters) => course.corners.length,
  ),
  ...
  course_distance: valueFilter(
    (course: CourseData, _: HorseParameters, extra: RaceParameters) => course.distance,
  ),
  ...
  ground_type: valueFilter(
    (course: CourseData, _: HorseParameters, extra: RaceParameters) => course.surface,
  ),
  hp_per: immediate({
    filterLte(
      regions: RegionList,
  ...
  rotation: valueFilter(
    (course: CourseData, _: HorseParameters, extra: RaceParameters) => course.turn,
  ),
  ...
  slope: immediate({
    filterEq(
      ...
      assert(
        CourseHelpers.isSortedByStart(course.slopes),
  ...
  track_id: valueFilter(
    (course: CourseData, _: HorseParameters, extra: RaceParameters) => course.raceTrackId,
  ),
