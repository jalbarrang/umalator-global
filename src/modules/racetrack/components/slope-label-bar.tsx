 ...
 */
export const SlopeLabelBar = React.memo<SlopeLabelBarProps>(
  ({ slopes, distance }) => {
    const elements = useMemo(() => {
        ...
        return (
          ...
          >
            ...
            <rect
              ...
              height="10%"
              width="100%"
              fill={colors.accent[colorIndex % 2]}
            />
            <SectionText
              id={isUphill ? 'uphill' : 'downhill'}
              w={s.length / distance}
            />
          </svg>
        );
      ...
      const slopeMarkers = slopes.map((s, i) => {
        ...
        if (
          s.start !== 0 &&
          (i === 0 || s.start !== slopes[i - 1].start + slopes[i - 1].length)
        ) {
          markedStart = true;
          nodes.push(
            <DistanceMarker
              ...
              up={
                i > 0 &&
                s.start - (slopes[i - 1].start + slopes[i - 1].length) <
                  distance * 0.05
              }
            />,
          );
