/**
 * Skill activation condition system
 * Conditions filter regions where skills can activate and provide dynamic runtime checks
 */
...
function withDefaultCond(r: RegionList | [RegionList, DynamicCondition]) {
  if (r instanceof RegionList) {
    return [r, () => true] as [RegionList, DynamicCondition];
  }
  return r;
}

/**
 * A Condition represents a skill activation requirement
 * It filters regions and may add runtime checks
 */
...
/**
 * An Operator wraps a Condition with a specific comparison operation
 */
...
export class EqOperator implements CmpOperator {
  samplePolicy: ActivationSamplePolicy;
  ...
  apply(regions: RegionList, course: CourseData, horse: RunnerParameters, extra: RaceParameters) {
    return withDefaultCond(this.condition.filterEq(regions, this.argument, course, horse, extra));
  }
}

export class NeqOperator implements CmpOperator {
  samplePolicy: ActivationSamplePolicy;
...
export class LtOperator implements CmpOperator {
  samplePolicy: ActivationSamplePolicy;
...
export class LteOperator implements CmpOperator {
  samplePolicy: ActivationSamplePolicy;
...
export class GtOperator implements CmpOperator {
  samplePolicy: ActivationSamplePolicy;
...
export class GteOperator implements CmpOperator {
  samplePolicy: ActivationSamplePolicy;
...
// Export the main Conditions dictionary (will be populated by importing the other condition files)
