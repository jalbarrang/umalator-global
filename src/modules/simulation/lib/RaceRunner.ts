...
import { BaseAccel, PhaseDeceleration, PosKeepMode, UphillBaseAccel } from './core/constants';
import { Strategy, StrategyHelpers } from './runner/types';
import { CompensatedAccumulator } from './utils/CompensatedAccumulator';
import { baseTargetSpeed, lastSpurtSpeed } from './physics/speed/SpeedCalculator';
import { GameTimerManager, InGameTimer } from './utils/Timer';
import { Rule30CARng } from './utils/Random';
import { PositionKeepState, SkillPerspective, SkillRarity, SkillType } from './skills/types';
import { CourseHelpers } from './course/CourseData';
import { calculateBaseAcceleration } from './physics/speed/AccelerationCalculator';
import { ApproximateMultiCondition, ApproximateStartContinue } from './ApproximateStartContinue';
import { RushedStateManager } from './behavior/special-states/RushedStateManager';
import type { DownhillModeManager } from './behavior/special-states/DownhillModeManager';
import type { DuelingManager } from './behavior/competition/DuelingManager';
import type { SpotStruggleManager } from './behavior/competition/SpotStruggleManager';
...
import type { ActiveSkill, CourseData, IPhase, PendingSkill, SkillEffect } from './core/types';
import type { HpPolicy } from './physics/health/HealthPolicy';
import type { IStrategy, RunnerParameters } from './runner/types';
...
export class RaceRunner {
  private rushedManager: RushedStateManager;
  private downhillManager: DownhillModeManager;
  private duelingManager: DuelingManager;
  private spotStruggleManager: SpotStruggleManager;

  accumulatetime: InGameTimer;
  pos: number;
  minSpeed: number;
  currentSpeed: number;
  targetSpeed: number;
  ...
  horse: { -readonly [P in keyof RunnerParameters]: RunnerParameters[P] };
  course: CourseData;
  ...
  umas: Array<RaceRunner>;
  isPacer: boolean;
  pacerOverride: boolean;
  posKeepMinThreshold: number;
  posKeepMaxThreshold: number;
  declare posKeepCooldown: InGameTimer;
  posKeepNextTimer: InGameTimer;
  ...
  competeFightTimer: InGameTimer;
  competeFightTargets: Set<RaceRunner>;

  // Lead Competition
  leadCompetition: boolean;
  leadCompetitionStart: number | null;
  leadCompetitionEnd: number | null;
  leadCompetitionTimer: InGameTimer;
  ...
  private conditionTimer: InGameTimer;
  ...
  constructor(params: {
    ...
    this.horse = cloneDeep(params.runner);
    ...
    this.rushedManager = new RushedStateManager({
      timerManager: this.timerManager,
      disabled: params.disableRushed ?? false,
      rng: this.rng,
    ...
    this.baseAccel = [0, 1, 2, 0, 1, 2].map((phase, i) =>
      calculateBaseAcceleration(i > 2 ? UphillBaseAccel : BaseAccel, this.horse, phase as IPhase),
    );
  ...
  initUmas(umas: Array<RaceRunner>) {
  ...
  step(dt: number) {
    ...
    this.hp.tick(
      {
        accumulatetime: this.accumulatetime,
        activateCount: this.activateCount,
        activateCountHeal: this.activateCountHeal,
        currentSpeed: this.currentSpeed,
        isLastSpurt: this.isLastSpurt,
        lastSpurtSpeed: this.lastSpurtSpeed,
        lastSpurtTransition: this.lastSpurtTransition,
        positionKeepState: this.positionKeepState,
        isDownhillMode: this.isDownhillMode,
        phase: this.phase,
        pos: this.pos,
        hp: this.hp,
        randomLot: this.randomLot,
        startDelay: this.startDelay,
        gateRoll: this.gateRoll,
        usedSkills: this.usedSkills,
        leadCompetition: this.leadCompetition,
  ...
  updatePacer(pacemaker: RaceRunner) {
    this.pacer = pacemaker;
  }

  getPacer(): RaceRunner | null {
  ...
  getUmaByDistanceDescending(): Array<RaceRunner> {
  ...
  updateCompeteFight() {
    ...
    const newTargets = new Set<RaceRunner>();
  ...
  updateLeadCompetition() {
    // First, update durations for all active spot struggles
    for (const uma of this.umas) {
      uma.spotStruggleManager.updateDuration(uma.position, this.dt);
    }

    // Then, check for new spot struggle triggers
    // (Only one uma needs to check - when conditions met, activate ALL in group)
    for (const uma of this.umas) {
      if (!uma.spotStruggleManager.canTrigger(uma.position)) {
        continue;
      }

      // Get threshold values from manager
      const distanceThreshold = uma.spotStruggleManager.getDistanceThreshold();
      const laneThreshold = uma.spotStruggleManager.getLaneThreshold();

      // Find umas with matching strategy within thresholds
      const umasWithinGap = this.umas.filter((other) => {
        if (other.posKeepStrategy !== uma.posKeepStrategy) {
          return false;
        }
        return (
          Math.abs(other.position - uma.position) <= distanceThreshold &&
          Math.abs(other.lane - uma.lane) < laneThreshold
        );
      });

      // Need at least 2 umas (including self)
      if (umasWithinGap.length >= 2) {
        // Activate spot struggle on ALL umas in the group
        for (const targetUma of umasWithinGap) {
          targetUma.spotStruggleManager.activate(targetUma.position);
        }
        // Once activated, no need to check other umas this frame
        break;
      }
    }
  }
