...
import { calculateMaxSpurtSpeed } from '../SpurtCalculator';
...
import type { IStrategy, RunnerParameters } from '@/modules/simulation/lib/runner/types';
...
import { calculateGutsModifier } from '@/modules/simulation/lib/core/formulas';
...
export class EnhancedHpPolicy {
  ...
  constructor(
    ...
    rng: PRNG,
    recalculateOnHeal: boolean = false,
  ) {
  ...
  private calculateBaseTargetSpeed(horse: RunnerParameters, phase: IPhase): number {
    ...
    return (
      this.baseSpeed * StrategyPhaseCoefficient[horse.strategy][phase] +
      (phase == 2
  ...
  private calculateMaxSpurtSpeed(runner: RunnerParameters): number {
    const DistanceProficiencyModifier = [1.05, 1.0, 0.9, 0.8, 0.6, 0.4, 0.2, 0.1];
  ...
  recover(modifier: number, state?: RaceState) {
    ...
    if (this.recalculateOnHeal && state && state.phase >= 2 && this.spurtParameters) {
      ...
      if (spurtDistance >= maxDistance) {
        ...
        const totalConsumeV3 = this.calcRequiredHp(
          this.baseTargetSpeed2,
          maxDistance - 60,
          true,
          false,
          false,
  ...
  private calcRequiredHp(
    ...
    return (
      (length / velocity) * baseConsumption * this.groundModifier * gutsModifier * statusModifier
    );
  }
  ...
  getLastSpurtPair(state: RaceState, maxSpeed: number, baseTargetSpeed2: number): [number, number] {
    ...
    const maxDistance = this.distance - state.pos;
    const spurtDistance = this.calcSpurtDistance(state, this.maxSpurtSpeed);
    const totalConsume = this.calcRequiredHp(
      this.maxSpurtSpeed,
      maxDistance - 60,
      true,
      false,
