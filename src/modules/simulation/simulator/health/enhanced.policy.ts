...
export class EnhancedHpPolicy implements HpPolicy {
  ...
  private calculateBaseTargetSpeed(horse: HorseParameters, phase: IPhase): number {
    ...
    if (this.recalculateOnHeal && state && state.phase >= 2 && this.spurtParameters) {
      ...
      if (spurtDistance >= maxDistance) {
        ...
        this.spurtParameters = {
          ...
          spDiff:
            this.currentHealth -
            this.calcRequiredHp(this.maxSpurtSpeed, maxDistance - 60, true, false),
          time: 0,
        };
      } else {
        ...
        const excessHp = this.currentHealth - totalConsumeV3;
  ...
  private calcSpurtDistance(state: IRaceState, targetSpeed: number): number {
    ...
    const excessHp = this.currentHealth - hpForBase;
  ...
  getLastSpurtPair(
    ...
    if (spurtDistance >= maxDistance) {
      ...
      this.spurtParameters = {
        distance: maxDistance,
        speed: this.maxSpurtSpeed,
        spDiff: this.currentHealth - totalConsume,
        time: 0,
      };
