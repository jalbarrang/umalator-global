...
export function ImportCodeDialog({
  ...
  const imageUrl = useMemo(
    () => getUmaImageUrl(decoded?.outfitId),
    [decoded?.outfitId],
  );
  ...
  const skillNames = useMemo(() => {
    ...
    return decoded.skills.map(
      (id) => skillCollection[id]?.name ?? `Unknown (${id})`,
    );
  }, [decoded?.skills]);
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Paste a RosterView export code to import a runner.
          </DialogDescription>
        </DialogHeader>
          ...
          {decoded && (
            <div className="flex flex-col gap-3 p-3 border rounded-md">
              ...
              <div className="grid grid-cols-5 gap-1 text-center">
                {['Speed', 'Stamina', 'Power', 'Guts', 'Wit'].map(
                  (label, i) => (
                    <div
                      key={label}
