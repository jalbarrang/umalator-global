...
export function UmaOddsBar(props: UmaOddsBarProps) {
  ...
  return (
    <p className={cn('min-w-[180px] text-xs leading-snug text-muted-foreground', className)}>
      <span className="font-mono font-semibold text-foreground tabular-nums">{formatPercent(chance)}</span> chance to
      get {target} from luck before the {PITY_PULLS}-pull spark.
    </p>
  );
}
