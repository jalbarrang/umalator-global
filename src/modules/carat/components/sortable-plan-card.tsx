      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        ...
        <div className="grid content-start gap-2 text-right">
          <div className="text-xs text-muted-foreground">
            Carats avail.{' '}
            <span className="font-medium font-mono text-foreground tabular-nums">{formatCarats(row.caratsAvailable)}</span>
          </div>
          {showPaid && (row.paidCaratsAvailable > 0 || row.paidBalanceAfter > 0) ? (
            <div className="text-xs text-muted-foreground">
              Paid pool{' '}
              <span className="font-medium font-mono text-foreground tabular-nums">{formatCarats(row.paidCaratsAvailable)}</span>
            </div>
          ) : null}
