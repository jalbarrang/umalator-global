...
function computeTrackStatThresholdModifier(
...
function computeLastSpurtTargetSpeed(
  courseContext: FrontendModel['courseContext'],
  horse: FrontendHorse
): number {
  ...
  const speedTerm =
    Math.sqrt(SPEED_TERM_COEFF * Math.max(adjustedSpeed, 0)) * distMod * SPEED_TERM_SCALE;
  const phase2BaseSpeed = baseSpeed * strategyCoeffs[2];
  const lateRaceBaseSpeed = phase2BaseSpeed + speedTerm;
  const gutsTerm =
    Math.pow(GUTS_TERM_BASE * Math.max(adjustedGuts, 0), GUTS_TERM_EXPONENT) * GUTS_TERM_SCALE;
  return (
    (lateRaceBaseSpeed + LAST_SPURT_BASE_RATIO * baseSpeed) * LAST_SPURT_MULTIPLIER +
    speedTerm +
    gutsTerm
  );
}
...
function parseConditionToken(
  token: string,
  context: Record<string, number | Set<number>>
): boolean {
...
function evaluateCondition(
  ...
  return condition
    .split('@')
    .some((orPart) => orPart.split('&').every((token) => parseConditionToken(token, context)));
}
...
function buildHorseRoomContext(
  ...
  const sameSkillCount = allHorses.filter((other) =>
    other.learned_skill_ids.includes(skillId)
  ).length;
...
export function buildFrontendRoom(
  ...
  for (const [index, data] of raceHorseInfo.entries()) {
    ...
    const rawStrategy = Number(
      data.running_style ?? trainedChara.rawData?.param?.runningStyle ?? 1
    );
    const strategy =
      rawStrategy === 1 && learnedSkillIds.includes(RUNAWAY_TRIGGER_SKILL_ID) ? 5 : rawStrategy;
    ...
    const aptStyleField =
      RUNNING_STYLE_TO_APTITUDE_FIELD[strategy] ?? RUNNING_STYLE_TO_APTITUDE_FIELD[1];
...
function canonicalizeRoom(room: FrontendRoom): FrontendRoom {
  return {
    ...
    teams: [...room.teams]
      ...
      .map((team) => ({
        ...team,
        horses: [...team.horses].sort(
          (a, b) =>
            a.frame_order - b.frame_order || a.card_id - b.card_id || a.strategy - b.strategy
        )
      }))
  };
}
...
function baseFeatureCount(model: FrontendModel): number {
  ...
  return (
    model.schema.numericFields.length +
    styleIds.length +
    gateNumbers.length +
...
function linear(
  row: number[],
  weight: TensorPayload | undefined,
  bias: TensorPayload | undefined
): number[] {
...
function countRecoveryBuckets(
  ...
  const counts = Object.fromEntries(
    SURROGATE_RECOVERY_BUCKETS.map((bucket) => [bucket, 0])
...
function computeSurrogateOutputs(
  horse: FrontendHorse,
  model: FrontendModel
  ...
  const input = buildSurrogateInputVector(
    horse,
    surrogate.inputFeatureNames,
    surrogate.recoveryValueBySkillId
  );
  ...
  const first = relu(
    linear(normalizedInput, surrogate.weights['net.0.weight'], surrogate.weights['net.0.bias'])
  );
  const second = relu(
    linear(first, surrogate.weights['net.3.weight'], surrogate.weights['net.3.bias'])
  );
  ...
  for (const [index, featureName] of surrogate.outputFeatureNames.entries()) {
    ...
    result[featureName] =
      featureName === 'surrogate_full_spurt_prob' || featureName === 'surrogate_no_spurt_prob'
        ? sigmoid(value)
        : value;
  }
  return result;
...
function computeSurrogateContextFeatureMap(
  ...
  const hpMarginRatio = Math.max(
    -1.5,
    Math.min(1.5, Number(outputs.surrogate_hp_margin_ratio ?? 0))
  );
...
function computeRaceMechanicsFeatureMap(
  ...
  const lastSpurtTargetSpeed = Number(horse.last_spurt_target_speed ?? 0);
  const spurtHpPerSecond =
    20 * ((lastSpurtTargetSpeed - baseSpeed + 12) ** 2 / 144) * hpGroundModifier * gutsHpModifier;
...
export function encodeRoom(
  room: FrontendRoom,
  model: FrontendModel
): { features: number[][][]; orderedHorses: FrontendHorse[] } {
  ...
  for (const field of model.schema.rankFields) {
    roomMaxLookup.set(
      field,
      Math.max(...allHorses.map((horse) => numericHorseField(horse, field)))
    );
  }
  ...
  const features = canonical.teams.map((team) => {
    ...
    for (const field of model.schema.rankFields) {
      opponentMaxLookup.set(
        field,
        Math.max(...opponentHorses.map((horse) => numericHorseField(horse, field)))
      );
    }
    const teamGateNumbers = team.horses.map((horse) => horse.frame_order + 1);
    const teamGateMean =
      teamGateNumbers.reduce((sum, value) => sum + value, 0) / Math.max(teamGateNumbers.length, 1);
    const teamGateSpread =
      teamGateNumbers.length > 0 ? Math.max(...teamGateNumbers) - Math.min(...teamGateNumbers) : 0;
    ...
    return team.horses.map((horse) => {
      ...
      for (const featureName of extraFeatureNames) {
        ...
        if (featureName.startsWith('opponent_rank_pct_')) {
          ...
          row.push(
            computeRankPctAgainst(
              opponentHorses.map((otherHorse) => numericHorseField(otherHorse, field)),
              numericHorseField(horse, field)
            )
          );
          continue;
        }
        ...
        if (featureName === 'capped_core_stat_count') {
          row.push(
            [horse.speed, horse.stamina, horse.pow, horse.guts, horse.wiz].filter(
              (stat) => stat >= 1200
            ).length
          );
          continue;
