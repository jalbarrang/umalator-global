...
function gelu(value: number): number {
  return (
    0.5 * value * (1 + Math.tanh(Math.sqrt(2 / Math.PI) * (value + 0.044715 * Math.pow(value, 3))))
  );
}
...
function linear(
  row: number[],
  weight: FrontendModel['weights'][string],
  bias: FrontendModel['weights'][string]
): number[] {
...
function mlpBlock(row: number[], prefix: string, model: FrontendModel): number[] {
  const linear1 = linear(
    row,
    model.weights[`${prefix}.0.weight`],
    model.weights[`${prefix}.0.bias`]
  );
  const act1 = linear1.map(gelu);
  const norm1 = layerNorm(
    act1,
    model.weights[`${prefix}.2.weight`].data,
    model.weights[`${prefix}.2.bias`].data
  );
  const linear2 = linear(
    norm1,
    model.weights[`${prefix}.4.weight`],
    model.weights[`${prefix}.4.bias`]
  );
  const act2 = linear2.map(gelu);
  return layerNorm(
    act2,
    model.weights[`${prefix}.6.weight`].data,
    model.weights[`${prefix}.6.bias`].data
  );
}
...
function scorer(row: number[], model: FrontendModel): number {
  ...
  const n1 = layerNorm(
    a1,
    model.weights['scorer.2.weight'].data,
    model.weights['scorer.2.bias'].data
  );
  const l2 = linear(n1, model.weights['scorer.4.weight'], model.weights['scorer.4.bias']);
  const a2 = l2.map(gelu);
  const n2 = layerNorm(
    a2,
    model.weights['scorer.6.weight'].data,
    model.weights['scorer.6.bias'].data
  );
...
export function predictEncodedRoom(
  ...
  const logits = horseEmbeddings.map((horse, index) => {
    ...
    if (model.modelConfig.modelType === 'deepsets') {
      return scorer(
        [...horse, ...teamMeans[teamIndex], ...teamMaxes[teamIndex], ...roomMean, ...roomMax],
        model
      );
    }
  ...
  const rankedIndices = [...probabilities.keys()].sort(
