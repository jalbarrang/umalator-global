...
import { RaceSolver, PendingSkill } from '@simulation/RaceSolver';
...
program
  .argument(
    '<horsefile1>',
    "path to a JSON file describing the first horse's parameters",
  )
  .argument(
    '<horsefile2>',
    "path to a JSON file describing the second horse's parameters",
  )
...
const skillDefs1 = desc1.skills
  .map((s) => buildSkillData(horse1, course, wholeCourse, s))
  .filter((s) => s != null);
const skillDefs2 = desc2.skills
  .map((s) => buildSkillData(horse2, course, wholeCourse, s))
  .filter((s) => s != null);

const seed =
  'seed' in opts ? opts.seed : Math.floor(Math.random() * (-1 >>> 0));
...
const triggers1 = skillDefs1.map((sd) =>
  sd.samplePolicy.sample(sd.regions, opts.nsamples, rng1),
);
const triggers2 = skillDefs2.map((sd) =>
  sd.samplePolicy.sample(sd.regions, opts.nsamples, rng2),
);
...
console.log(
  'median:\t' +
    (gain.length % 2 == 0
      ? (gain[mid - 1] + gain[mid]) / 2
      : gain[mid]
    ).toFixed(2),
);
console.log(
  'mean:\t' + (gain.reduce((a, b) => a + b) / gain.length).toFixed(2),
);
...
opts.thresholds.forEach((n) => {
  console.log(
    '≥' +
      n.toFixed(2) +
      ' | ' +
      ((gain.reduce((a, b) => a + +(b >= n), 0) / gain.length) * 100).toFixed(
        2,
      ) +
      '%',
  );
