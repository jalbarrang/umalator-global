...
import { Region } from '@simulation/Region';
import { Rule30CARng } from '@simulation/Random';
import { SkillRarity, PendingSkill, RaceSolver } from '@simulation/RaceSolver';
import { NoopHpPolicy } from '@simulation/HpPolicy';
import {
  ImmediatePolicy,
  RandomPolicy,
} from '@simulation/ActivationSamplePolicy';
...
cli.options((program) => {
  program
    .addOption(
      new Option(
        '-N, --nsamples <N>',
        'number of random samples to use for skills with random conditions',
      )
        .default(500)
    ...
    .option(
      ...
      (value, _) => parseInt(value, 10) >>> 0,
    )
    .option(
      '--enable-wisdom-checks',
      'base skill activation on random checks dependent on wisdom',
    )
    .addOption(
      new Option(
    ...
    .option(
      '--dump',
      'instead of printing a summary, dump data. intended to be piped into histogram.py.',
    )
    .option(
      '--csv [first_col]',
      'print data as a CSV row (intended for batch scripting)',
    );
});
cli.run(
  (
    ...
    const skillActivationChance = cliOptions.enableWisdomChecks
      ? Math.max(100.0 - 9000.0 / horse.wisdom) / 100.0
      : 1.0;
    ...
    for (let i = cliSkills.length; --i >= 0; ) {
      ...
      if (ef.some((e) => e.modifier < 0)) {
        const debuffEffects = [];
        debuffs.push(
          Object.assign({}, cliSkills[i], { effects: debuffEffects }),
        );
    ...
    for (let i = 0; i < nsamples; ++i) {
      ...
      for (
        let i = 0;
        i < defSkills.length + cliSkills.length + debuffs.length;
        ++i
      ) {
      ...
      function wisdomCheck1(sd: SkillData, i: number) {
        return (
          sd.rarity == SkillRarity.Unique ||
          skillCheckRolls1[i] <= skillActivationChance
        );
      }
      ...
      defSkills
        .filter(wisdomCheck1)
        .forEach((sd, sdi) => addSkill(skills1, sd, triggers[sdi], i));
      cliSkills
        .filter((sd, sdi) => wisdomCheck1(sd, sdi + defSkills.length))
        .forEach((sd, sdi) =>
          addSkill(skills1, sd, triggers[sdi + defSkills.length], i),
        );
      ...
      function wisdomCheck2(sd: SkillData, i: number) {
        return (
          sd.rarity == SkillRarity.Unique ||
          skillCheckRolls2[i] <= skillActivationChance
        );
      }
      ...
      defSkills
        .filter(wisdomCheck2)
        .forEach((sd, sdi) => addSkill(skills2, sd, triggers[sdi], i));
      debuffs
        .filter((sd, sdi) =>
          wisdomCheck2(sd, sdi + defSkills.length + cliSkills.length),
        )
        .forEach((sd, sdi) =>
          addSkill(
            skills2,
            sd,
            triggers[sdi + defSkills.length + cliSkills.length],
            i,
          ),
        );
    ...
    const median =
      gain.length % 2 == 0 ? (gain[mid - 1] + gain[mid]) / 2 : gain[mid];
    ...
    if (cliOptions.csv) {
      ...
      cliOptions.thresholds.forEach((n) => {
        cols.push(
          (
            (gain.reduce((a, b) => a + +(b >= n), 0) / gain.length) *
            100
          ).toFixed(2) + '%',
        );
      });
