...
import { CourseData, CourseHelpers } from '@simulation/CourseData';
import { Aptitude, HorseParameters, Strategy } from '@simulation/HorseTypes';
...
export function buildHorseParameters(
  horseDesc,
  course: CourseData,
  mood: Mood,
  ...
  return Object.freeze({
    ...
    wisdom:
      ...
      StrategyProficiencyModifier[
        parseAptitude(horseDesc.strategyAptitude, 'strategy')
      ],
...
export class ToolCLI {
  ...
  constructor() {
    ...
    this.program
      ...
      .option(
        '-s, --skill <id>',
        'skill to test',
        (value, list) => list.concat([parseInt(value, 10)]),
        [],
      )
      .option(
        '--skills <ids>',
      ...
      .option(
        '--position-keep <pacer>',
        'load a horse from the <pacer> JSON file to simulate position keep (by default, uses a nige version of the horse in <horsefile> with no skills) (position keep is not simulated for nige/oonige)',
      )
      .option('--no-position-keep', 'disable position keep simulation')
      .addOption(
        new Option(
          '--timestep <dt>',
          'integration timestep in seconds (can be an integer, decimal, or fraction)',
        )
          .default(1 / 60, '1/60')
  ...
  handleRun(horsefile: string, opts: any) {
    ...
    const horse = buildHorseParameters(
      horseDesc,
      course,
      opts.mood,
      opts.ground,
    ...
    if (typeof opts.positionKeep == 'string') {
      ...
      pacerHorseParams = buildHorseParameters(
        pacerDesc,
        course,
        opts.mood,
        opts.ground,
    ...
    function getPacer(rng: PRNG) {
      ...
      if (
        horse.strategy != Strategy.Nige &&
        horse.strategy != Strategy.Oonige &&
        opts.positionKeep !== false
      ) {
