...
const rdRaw =
  rdResult.status === 'fulfilled' ? rdResult.value : (rdResult.reason?.stdout?.toString() ?? '');
...
ln(`| Unused dev dependencies | ${s.unused_dev_dependencies ?? 0} | 🟡 warn |`);
...
if (fallow.dupes) {
  ...
  ln();
  ln(
    `**${dupes.clone_groups.length} clone groups · ${dupes.stats.duplicated_lines} lines · ${dupes.stats.duplication_percentage.toFixed(1)}%**`
  );
  ln();
  ...
  for (const g of sorted.slice(0, 15)) {
    ...
    const files = g.instances.map((i) => `\`${i.file}:${i.start_line}–${i.end_line}\``).join(' ↔ ');
...
ln();
ln(
  `**Score: ${rd.summary.score}/100 (${rd.summary.scoreLabel})** · ${rd.summary.totalDiagnosticCount} diagnostics`
);
ln();
...
console.log(
  `  ${totalIssues} total issues (fallow: ${s.total_issues}, react-doctor: ${rd.summary.totalDiagnosticCount})`
);
